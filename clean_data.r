...
# excluding individuals with age less than 8 years old as they were not eligible for dxa measurement
df_demograph <- df_demograph %>% filter(RIDAGEEX >= 8 * 12)

# removing missing age data
df_demograph <- df_demograph %>% filter(!is.na(RIDAGEEX))
...
df_body_measur <- df_body_measur %>% filter(!is.na(BMXWT) & is.na(BMIWT))

# removing missing stature values and stature values that are not measured in appropriate condition
df_body_measur <- df_body_measur %>% filter(!is.na(BMXHT) & is.na(BMIHT))

# removing missing upper leg length values and values that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXLEG) & is.na(BMILEG))

# removing missing maximum calf values and values that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXCALF) & is.na(BMICALF))

# removing missing arm length values and values that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXARML) & is.na(BMIARML))

# removing missing arm circumference values and values that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXARMC) & is.na(BMIARMC))

# removing missing waist circumference values and ones that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXWAIST) & is.na(BMIWAIST))

# removing missing thigh circumference values and ones that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXTHICR) & is.na(BMITHICR))

# removing missing triceps skin-fold values and values that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXTRI) & is.na(BMITRI))

# removing missing Sub-scapular Skin-fold values and ones that couldn't be obtained
df_body_measur <- df_body_measur %>% filter(!is.na(BMXSUB) & is.na(BMISUB))
...
df_dxa <- df_dxa %>% filter(DXAEXSTS == 1 & DXITOTST == 0 & DXITOTBN == 0)